...
contract("CarRentalPlatform", (accounts) => {
  ...
  describe("Add user and car", () => {
    ...
    it("adds a car", async () => {
      ...
      assert.equal(car.imgUrl, "example url", "Problem with img ur]");
  ...
  describe("Deposit token and make payment", () => {
    ...
    it("makes a payment", async () => {
      ...
      assert.equal(
        user.debt,
        0,
        "Somehting went wrong while trying to make the payment"
      );
    });
  });
  ...
  describe("edit car", () => {
    it("should edit an existing car's metadata with valid parameters", async () => {
      ...
      assert.equal(car.salefee, newSaleFee, "Problem with witing sale fee");
    });
